...
"""
Builds the final training data for the email-recommendation RAG chatbot.
from_work is handled by user profile, NOT in training.

Output: data/train.csv, data/val.csv
"""
...
# ============================================================
# BALANCED DATA (Reduce dominant classes)
# ============================================================
ENRON_CAP       = 1500  # Reduced from 5000
SYNTHETIC_COUNT = 800   # Reduced from 4000

random.seed(SEED)


def load_enron_to_other(n: int, today: datetime) -> pd.DataFrame:
    print(f"Loading Enron corpus for 'other' ({n} samples)...")
    ...
    df = df.dropna(subset=[text_col]).sample(n=min(n, len(df)), random_state=SEED).reset_index(drop=True)
...
def load_jason_subset(today: datetime) -> pd.DataFrame:
    ...
    out = pd.concat([df[["text", "label"]].reset_index(drop=True), extras_df.reset_index(drop=True)], axis=1)
...
def gen_deadline(today: datetime) -> dict:
    ...
    return {"text": body, "subject": subject, "sender": f"noreply@{platform.lower().replace(' ', '')}.com", "date": due_date}
...
def gen_interview_call(today: datetime) -> dict:
    ...
    return {"text": body, "subject": subject, "sender": f"hr@{company.lower().replace(' ', '')}.com", "date": interview_date}
...
def gen_confirmation_email(today: datetime) -> dict:
    ...
    subjects = [
        f"Order Confirmation #{order_id} - {shop}",
        f"Your {shop} order #{order_id} is confirmed",
        f"Thanks for your order #{order_id} from {shop}",
    ]
    subject = random.choice(subjects)
    body = (
        f"Hi {name},\n\n"
    ...
    return {"text": body, "subject": subject, "sender": f"orders@{shop.lower()}.com", "date": order_date}

def gen_otp(today: datetime) -> dict:
    name = random.choice(FIRST_NAMES)
    service = random.choice(["Amazon", "Google", "Facebook", "Bank", "PayPal"])
    code = ''.join([str(random.randint(0, 9)) for _ in range(6)])
    date = rand_date_str(today, -2, 0)
    subjects = [
        f"Your verification code: {code}",
        f"OTP for {service} login",
        f"Security code: {code}",
    ]
    subject = random.choice(subjects)
    body = (
        f"Hi {name},\n\n"
        f"Your one-time verification code for {service} is: {code}\n\n"
        f"This code will expire in 5 minutes.\n\n"
        f"Never share this code with anyone.\n\n"
        f"Regards,\n{service} Security Team"
    )
    return {"text": body, "subject": subject, "sender": f"security@{service.lower().replace(' ', '')}.com", "date": date}
...
def gen_expired_email(today: datetime) -> dict:
    ...
    return {"text": body, "subject": subject, "sender": f"noreply@{platform.lower().replace(' ', '')}.com", "date": past_date}

def gen_other(today: datetime) -> dict:
    name = random.choice(FIRST_NAMES)
    date = rand_date_str(today, -30, 0)
    subjects = ["General inquiry", f"Hello from {name}", "Checking in", "Quick question"]
    subject = random.choice(subjects)
    body = f"Hi,\n\nThis is a general email. Nothing specific to report.\n\nRegards,\n{name}"
    return {"text": body, "subject": subject, "sender": f"user{random.randint(1,100)}@example.com", "date": date}
...
def load_and_prepare():
    ...
    enron_df = load_enron_to_other(ENRON_CAP, today)
